          ...
          {experiences.map((exp, idx) => (
            <div key={exp.id} className="relative">
              ...
              <div className="absolute -left-[41px] md:-left-[57px] top-4 flex items-center justify-center">
                ...
                {/* Central solid dot */}
                <div className="w-4 h-4 rounded-full bg-[#050816] border-[3px] border-primary shadow-[0_0_8px_#00E5FF] z-10" />
              </div>
              ...
              >
                <GlassCard className="flex flex-col gap-4 relative overflow-hidden group">
                  ...
                  <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 border-b border-white/5 pb-4">
                    <div>
                      <h3 className="font-display font-extrabold text-lg md:text-xl text-white uppercase tracking-wide">
                        {exp.role}
                      </h3>
                      <div className="flex items-center gap-2 text-primary font-mono text-xs mt-1">
                        <Building2 size={13} />
                        <span className="font-bold">{exp.company}</span>
                        <span className="text-[10px] text-text-muted px-1.5 py-0.5 rounded bg-white/5 border border-white/10 uppercase tracking-widest">
                          {exp.type}
                        </span>
                      </div>
                    </div>

                    <div className="flex items-center gap-1.5 text-text-secondary font-mono text-xs bg-white/5 border border-white/5 px-3 py-1.5 rounded-xl self-start sm:self-auto shadow-inner">
